...
class ProposeMeasureApp:
    ...
    def handle_submit(self):
        ...
        form_data = {
            ...
            "series_yr": self.window.seriesyearInput.date().toPyDate(),
    ...
    def save_form_data(self):
        ...
        form_data = {
            ...
            "series_yr": self.window.editSeriesDateInput.date().toPyDate(),
    ...
    def handle_attach_file(self, window, inputField):
        ...
        if new_files:
            ...
            if len(verified_files) < len(all_files):
                missing = set(os.path.basename(f) for f in all_files) - set(os.path.basename(f) for f in verified_files)
                display_text += f" (Missing: {', '.join(missing)})"
            inputField.setText(display_text)
            inputField.setToolTip(";".join(verified_files))  # Store full paths
        ...
        for filename in filenames_str.split(";"):
            ...
            if os.path.exists(full_path):
                try:
                    ...
                    print(f"[ERROR] Could not open {full_path}: {str(e)}")
                    QMessageBox.warning(window, "File Error", f"Could not open '{filename}'.")
            else:
